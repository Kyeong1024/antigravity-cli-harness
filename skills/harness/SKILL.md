...
5. **사용자 요청 언어 및 컨텍스트에 맞게 작성 언어를 결정한다.** — 사용자가 영어로 요청을 하거나, 프로젝트의 글로벌 협업 환경, 코드베이스 주석/문서 표준이 영어인 경우, 새로 생성하는 서브에이전트(`agent.json`의 `description` 및 `systemPromptSections` 등), 스킬(`SKILL.md` 본문 및 YAML frontmatter `description` 등), 그리고 오케스트레이터의 모든 텍스트를 영문(English)으로 작성한다.
...
전문성·병렬성·컨텍스트·재사용성 4축으로 판단한다. 상세 기준표는 `rules/agent-design-patterns.md`의 "에이전트 분리 기준" 참조. 기존 서브에이전트와의 중복·재사용 검토는 Phase 3-0에서 다룬다.

### Phase 3: 서브에이전트 정의 생성

#### 3-0. 기존 서브에이전트 중복 검토

신규 서브에이전트 생성 전, `프로젝트/.agents/plugins/*/agents/`의 기존 `agent.json`과 중복 여부를 확인한다. 하네스를 반복 구축하다 보면 역할이 겹치는 서브에이전트가 다른 이름으로 누적되기 쉽다.

> 중복 분류 기준과 재사용 설계는 `rules/agent-design-patterns.md`의 "서브에이전트 재사용 설계" 참조.
...
- **언어 일관성**: Phase 1에서 결정된 작성 언어(한국어 또는 영어)에 맞게 `description`과 `systemPromptSections` 내부의 system prompt를 일관되게 작성한다. 영어 프로젝트 또는 영어 요청의 경우, 모든 프롬프트와 설명을 자연스러운 영문(Technical English)으로 작성해야 한다.
...
신규 스킬 생성 전, `프로젝트/.agents/plugins/*/skills/`의 기존 스킬과 중복 여부를 확인한다. 하네스를 반복 구축하다 보면 기능이 겹치는 스킬이 다른 이름으로 누적되기 쉽다.

> 중복 분류 기준과 일반화 패턴은 `rules/skill-writing-guide.md`의 "스킬 재사용 설계" 참조.
...
- [ ] 기존 에이전트/스킬과 충돌 없음
- [ ] 신규 서브에이전트 생성 전 기존 서브에이전트 중복 검토 완료 (Phase 3-0)
